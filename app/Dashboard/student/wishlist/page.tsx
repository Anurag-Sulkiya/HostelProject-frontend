...
export default function WishlistPage() {
  ...
  const fetchWishlist = async () => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:5000/api/students/wishlist/${profileId}`,
        {
  ...
  const removeFromWishlist = async (hostelId: string) => {
    ...
    try {
      await axios.post(
        "http://localhost:5000/api/students/wishlist/remove",
        { hostelId },
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }
  ...
  const submitWishlist = async () => {
    ...
    try {
      await axios.post(
        "http://localhost:5000/api/students/wishlist/submit",
        {},
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }
  ...
  const takeAdmission = async (hostelId: string) => {
    ...
    try {
      await axios.post(
        "http://localhost:5000/api/students/take-admission",
        { hostelId },
        {
          headers: {
  ...
  const markAsNotInterested = async (hostelId: string) => {
    try {
      const response = await axios.post(
        "http://localhost:5000/api/students/not-interested",
        { hostelId },
        {
          headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
        }
  ...
  const uploadAdmissionReceipt = async (
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/students/upload-receipt",
        formData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const submitFeedback = async () => {
    ...
    try {
      await axios.post(
        "http://localhost:5000/api/students/submit-feedback",
        {
          hostelId: selectedHostel._id,
