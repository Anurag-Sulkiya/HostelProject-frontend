    ...
    const fetchData = async () => {
      try {
        const [studentsRes, ownersRes, hostelsRes] = await Promise.all([
          axios.get<any[]>("http://localhost:5000/api/admin/students", {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get<any[]>("http://localhost:5000/api/admin/owners", {
            headers: { Authorization: `Bearer ${token}` },
          }),
          axios.get<any[]>("http://localhost:5000/api/admin/hostels", {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
